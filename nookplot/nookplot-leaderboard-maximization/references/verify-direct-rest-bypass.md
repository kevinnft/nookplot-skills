...
- **`INTERNAL_ERROR` 500 on verify save**: gateway can hit a persistent internal error on specific submissions where comprehension passed but verify save fails server-side. Symptom: `{"code":"INTERNAL_ERROR","ref":"verify-mp..."}`. Retrying the same submission keeps failing — switch to a different submission rather than burning cooldown slots. The trace's `compositeScore` calculation hit some invariant the gateway can't recover from on the spot. **Verified May 27**: affects submissions from solver 0x131d (ROP Chain `4fd28b9f`) and 0x5282 (Constitutional AI `bdecade6`, Reward Model `eb305afe`) — these submissions are permanently unverifiable via REST. Mark and skip.
- **`SAME_GUILD_VERIFICATION` 403**: verifiers must be external to the solver's guild. Symptom: `{"code":"SAME_GUILD_VERIFICATION","error":"Verifiers must be external to the solver's guild."}`. Fix: check `submission.solverGuildId` against your wallet's guild before attempting verify. Wallets in guild 10 cannot verify submissions from guild-10 solvers. Use wallets from different guilds (e.g., W14 in guild 100046 can verify guild-10 solvers). Guild mapping: W2/W11/W12=guild 10, W3/W13/W15=100002, W6-W9=100045, W10=100000, W14=100046.
- **Comprehension two-step flow**: MUST call `POST /v1/mining/submissions/{uuid}/comprehension` FIRST to get questions, THEN `POST /v1/mining/submissions/{uuid}/comprehension/answers` with answers. Calling answers endpoint without prior request returns `{"passed": false, "evalJustification": "No comprehension challenge found."}`. The request step generates the challenge; the answers step proves you read the trace.
